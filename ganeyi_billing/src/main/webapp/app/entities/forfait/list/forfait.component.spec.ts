...
describe('Forfait Management Component', () => {
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      imports: [RouterTestingModule.withRoutes([{ path: 'forfait', component: ForfaitComponent }]), HttpClientTestingModule],
  ...
  it('Should call load all on init', () => {
    ...
    expect(comp.forfaits?.[0]).toEqual(expect.objectContaining({ id: 'ABC' }));
  });

  describe('trackId', () => {
    it('Should forward to forfaitService', () => {
      const entity = { id: 'ABC' };
      jest.spyOn(service, 'getForfaitIdentifier');
      const id = comp.trackId(0, entity);
      expect(service.getForfaitIdentifier).toHaveBeenCalledWith(entity);
