...
type RestOf<T extends IRequest | NewRequest> = Omit<T, 'requestDate'> & {
...
export class RequestService {
  ...
  protected convertDateFromClient<T extends IRequest | NewRequest | PartialUpdateRequest>(request: T): RestOf<T> {
    return {
      ...request,
      requestDate: request.requestDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restRequest: RestRequest): IRequest {
    return {
      ...restRequest,
      requestDate: restRequest.requestDate ? dayjs(restRequest.requestDate) : undefined,
    };
  }
